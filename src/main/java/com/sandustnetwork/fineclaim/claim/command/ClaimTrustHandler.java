...
final class ClaimTrustHandler {

    private final ClaimService claimService;
    private final PermissionChecker permissionChecker;

    ClaimTrustHandler(ClaimService claimService, PermissionChecker permissionChecker) {
    ...
    void handle(Player player, String[] args) {
        ...
        if (args.length < 1) {
            FineClaimMessages.sendWarning(player, "Usage: /claim trust <player>");
            return;
        }
