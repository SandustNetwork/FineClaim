...
public final class UntrustCommand implements BasicCommand {

    private final ClaimService claimService;
    private final PermissionChecker permissionChecker;

    public UntrustCommand(ClaimService claimService, PermissionChecker permissionChecker) {
    ...
    public void execute(CommandSourceStack source, String[] args) {
        ...
        if (args.length < 1) {
            FineClaimMessages.sendWarning(player, "Usage: /untrust <player>");
            return;
        }
        ...
        ClaimOperationResult result = claimService.untrustPlayer(chunk, player.getUniqueId(), targetId);
